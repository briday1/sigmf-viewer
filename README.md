...
Batch mode renders a high-resolution waterfall for an entire recording without
starting the interactive server. Actions can run for one catalog row or the
whole workspace; collections expand across every member and channel. The
renderer uses bounded-memory chunks, includes every source interval, and
writes deterministic PNGs under `outputs/`, allowing Sigvue to recognize
completed work after a restart.

This 2400×1600 LTE uplink result was generated by the viewer's standard batch
pipeline with FFT 256 and 50% overlap:

![High-resolution full-recording LTE uplink waterfall produced in batch mode](figures/batch-lte-uplink-waterfall.png)
